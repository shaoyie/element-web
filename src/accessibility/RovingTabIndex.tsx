...
const refSorter = (a: Ref, b: Ref): number => {
  ...
  // Add null checks to prevent crashes when refs are being unmounted
  if (!a.current || !b.current) {
    // If either ref is null, treat them as equal to avoid crashes
    // The null refs will be cleaned up by the Unregister action
    if (!a.current && !b.current) return 0;
    if (!a.current) return 1; // Move null refs to the end
